...
```bash
pi install git:github.com/kurumi1ksllq/pi-workflow@v1.13.2
```
...
- `Ctrl+O` 展开完整启动信息
- `pi list` 的 **User packages** 里应含 `pi-workflow@v1.13.2` 与清单里的那些包，且每项都带安装路径

**没生效就按顺序查**：第 2 步的凭据配了吗 → `pi list` 里有 `pi-workflow@v1.13.2` 吗
...
```bash
pi install git:github.com/kurumi1ksllq/pi-workflow@v1.13.2   # 1. 换版本
pi                                                           # 2. 启动：提示补了什么
pi                                                           # 3. 再启动一次：新清单里的包装上
```
...
pi install git:github.com/kurumi1ksllq/pi-workflow@v1.13.2   # 2. 换版本
...
```bash
pi install git:github.com/kurumi1ksllq/pi-workflow@v1.13.2   # 1. 换到钉版本的清单
...
| 情况 | 怎么办 |
| --- | --- |
| 你的版本太老（早于 v1.9.0，包里还没有自动更新逻辑） | 手动跑一次：`pi install git:github.com/kurumi1ksllq/pi-workflow@v1.13.2` 之后就不用管了 |
