...
- **子代理段两条新纪律：review 只跑一次 + 派 reviewer 必须给边界。** 依据是本机 42 次
  ...
  累计 5.3 小时。拆开看，**97-100% 的时间花在模型生成上，工具执行只占 0-11%** ——
  「reviewer 要读很多文件所以慢」是错的；慢在等模型吐字，而 `reviewer.md` 的
  `thinking: high` 让 reasoning 占输出 token 的 74-98%。
  最慢一次（c2d29d0b）108 轮里 80 次 `grep` + 72 次 `read`、累计重发上下文 440 万 token
  （cacheRead 4,444,096）—— 任务是「live Prompt Audit」，没有停止条件，reviewer 规则又写明
  「不许发明问题」，于是它一路扫到超时。**无边界任务 = 无限探索**，这是最大的单项浪费。
  原来的「最多 3 轮」把单任务最坏情况放大到 45 分钟，改为**只跑一次**：它报的问题修完即可，
  只有操作者明确要求才重派。
- 新规矩要求派单时写明四件事：审查范围（哪几个文件/哪个 diff）、禁止项（不许全仓 `grep`、
  不许探索目录、不许读范围外文件、不许跑命令）、输出上限（≤300 字、按 P0/P1/P2 列）、
  `timeoutMs` ≤ 600000。默认的 30 分钟只会让人白等。
- `scripts/test-extension.mjs` 补两条断言护住新段，负向验证过：删掉该段 → 测试断出两条。
- ⚠️ 本条**不改** `subagents.disableThinking`。它已经是 `true` 且实测生效
  （子会话 `thinking_level_change` = `off`），但 reasoning 占比仍有 80%+ —— 说明该开关清的是
  子代理定义里的 thinking 后缀，不是上游模型自身的推理输出，**不要再拿它当速效药**。
- **档位 tier-power → tier-std 的对照做了，结论是「不换」。** 交替 A/B 三轮（同任务、同边界、
  `timeoutMs=600000`）：tier-power `223 / 228 / 55s`，tier-std `64 / 138 / 117s`。
  输出速率上 std 确实稳定快 2-3 倍（tok/s：std 60-110 vs power 20-35），但**端到端 wall time
  抖动极大**（第三轮反向），单次任务说明不了问题。真正决定不换的理由是设计层的：团队里
  实现绝大多数跑在 `tier-std`（`defaultModel`），把 reviewer 也换成 `tier-std` 就等于
  **同一个模型审自己写的代码，盲区一模一样** —— 那正是 `agentOverrides` 把审查类钉到另一个档位
  的原因。**异模型审查的价值高于那点速度。**
- **由此定论：压 review 耗时只剩一个真杠杆 —— 任务边界。** 无边界任务会连跑上百轮生成，
  每轮都带着 80% 的 reasoning，于是「生成慢」被轮数放大成 30 分钟。边界一旦写死，
  轮数从 108 掉回个位数，thinking 占比就不再是瓶颈。所以本条的新纪律实际上同时解决了
  原来以为是两个问题的第 2 项 —— 不是「压 thinking」，而是「别让它跑那么多轮」。

## v1.13.4
- **网关档位上下文窗口提到 512k，压缩阈值同批调整**。四个档位从
  `128000 / 128000 / 272000 / 128000` 改为 `512000 / 512000 / 512000 / 256000`
  （免费档上游只有 262144，单独给 256000）。`compaction.reserveTokens` 从 `32768`
  改为 `65536`，触发点从「128k−32k = 96k」挪到「512k−64k = **448k**（87.5%）」。
  `reserveTokens` 的基准是模板里最大的 `maxTokens`（tier-max 的 64000）—— pi 的触发条件
  是 `contextTokens > contextWindow − reserveTokens`，触发那一刻必须还给模型留得下它最大的输出，
  否则真到上限会是上游硬报错而不是提前压缩。**两处是一对数，改一边不改另一边 = 触发比例悄悄漂掉**。
- **新增两类存量迁移**（`migrateStaleContextWindows` / `migrateStaleCompactionReserve`）。
  合并规则里「成员已有档位定义一律不动」「compaction 只补缺」是为了不覆盖成员自己的调整，
  副作用是**团队改模板推不到老成员身上** —— 他 models.json 里睡着旧模板写进去的 128000，
  settings 里睡着 32768，改模板只对新人生效。迁移的判据收得很紧：**只有当前值正好等于
  旧模板发出去的那个数字时才改**，成员自己调过的（哪怕只差 1）一律不动；窗口迁移还额外要求
  「模板里该档位的现值 == 迁移表记的新值」，防迁移表本身过期。只动模板自己声明的 provider。
- **healthCheck 加一条跨文件对账**：`agent-settings.json` 的 `reserveTokens` 不许小于
  `models.template.json` 里最大的 `maxTokens`。两处分居两个文件，靠记性必漂，只能机械拦。
- 测试补场景 12（窗口迁移：刷旧值 / 不动手改值 / 幂等）与场景 13（reserve 迁移 + 判据收紧），
  负向验证过：让迁移失效 → 场景 12 断出三条、场景 13 断出一条；把 reserve 调到 20000 → healthCheck 报警。

## v1.13.3
- **`team/RULES.md` 子代理段新增「oracle 只用于方案拿不准」纪律**。
  依据：2026-09-22 那批 `gpt-5.6-sol` 调用共 29 次，其中 26 次实际在做「实现完成后的独立审查」
  —— 那是 `reviewer` 的活。oracle 档是全链路最贵的一档：**缓存读单价是标准档的 166 倍、
  输出单价 67 倍**，一次约等于 50 次标准档调用。
  新规矩给了可操作判据：「能不能把问题写成『A 还是 B，为什么』——写不出就别派 oracle」，
  并明确「重要性不是判据，决策本身有没有分歧才是」。
  同时往 `scripts/test-extension.mjs` 加了断言护这段（负向验证：删掉该段测试 exit 1）。
- **`context-thrift` 团队默认开启工具声明裁剪（第二层）**。
  实测：工具定义每轮重发、占长会话上下文约 18~20%；开启后工具声明
  **41,609 → 22,832 字符（−45%）**。2026-09-23 20:08 起本机已开了两天、稳。
  `keep` 名单保持含 `subagent` 与 `maxTools` 兜底两道保险。
  已知边界写进配置注释：裁剪只作用于请求体的 tools 数组，提示词里的 `<tools>` 列表同步不了
  （pi 从自身 AgentSession 状态重渲染），模型可能以为被裁的工具还在 —— 实测启用期间没出现过
  调用被裁工具而失败。第三层（历史工具输出降级）**仍默认关**：它引入滑动边界会逐轮改变前缀、
  打掉 prompt cache（cacheRead 占全部 token 的 95%、单价只有全价 2%），要开必须先 A/B。
