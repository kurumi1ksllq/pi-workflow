...
扩展在启动时，把这些包里**当前还没有的**补进**全局设置** `~/.pi/agent/settings.json`
（团队全员是全局装，清单只有落到这里才生效），并在 stderr 提示一条。
做法和其他同步一样：**只补不删、不动已有的、重复调用安全**。
...
- 项目 `.pi/settings.json` 里如果手写过同一个包，扩展不会去删它（只补不删）
