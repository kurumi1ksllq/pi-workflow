...
| 装法 | 命令 | 落在哪 | 谁适合 |
| --- | --- | --- | --- |
| 项目级 | `pi install -l <源>` | 项目 `.pi/settings.json`（进 git） | 团队项目仓库 —— 新人 clone 后**自动**得到 |
| 全局级 | `pi install <源>` | `~/.pi/agent/settings.json` | 个人 —— 一次装，**本机所有项目**通用 |

两者可以并存：**同一个包同时出现在两处时功能正常**（不重复注入），代价只是磁盘上多一份 clone。
建议团队项目仓库里保留项目级配置（保证新成员零操作），个人再装一份全局（换到没接入的项目也能用）。
